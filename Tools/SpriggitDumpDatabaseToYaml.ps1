...
foreach ($database in $Global:Databases) {
  ...
  & "$ENV:TOOL_PATH_SPRIGGIT" serialize --InputPath "./Staging/$database" --OutputPath "./Spriggit/$database" --Check --GameRelease Starfield --PackageName Spriggit.Yaml --DataFolder "$ENV:STEAM_DATA_FOLDER"
}
